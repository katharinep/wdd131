...
function validateForm(event) {
    ...
    // the default behavior for a form submit is to try and navigate 
    // to another page where the form would be processed, if a url is 
    // not provided it will reload the current page. This sometimes is 
    // not desirable behavior. One case when we might do this is if we 
    // think there is bad data in the form.
    // To keep it from happening we can can call e.preventDefault()
    // You should always give feedback to the user about what whet 
    // wrong so they can fix it. We will store the error messages here
    const errors = [];
    // start by assuming the form is valid.
    let isValid = true;
    // add our validations here
    if (theForm.payment.value === "creditCard") {
        if (theForm.creditCardNumber.value !== "1234567891234567") {
            isValid = false;
